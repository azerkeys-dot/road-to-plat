...
const Inscription: React.FC = () => { const [pseudo,setPseudo]=React.useState(''); const [dpmUrl,setDpmUrl]=React.useState(''); const [discord,setDiscord]=React.useState(''); const [rank,setRank]=React.useState<Rank>('Unranked'); const [submitting,setSubmitting]=React.useState(false); const [players,setPlayers]=React.useState<Player[]|null>(null);
  ...
  <Card title={<><Users size={18}/> Joueurs inscrits ({players?.length ?? 0})</>}>
    <div className='overflow-x-auto'>
      <table className='w-full text-sm'>
        ...
        <tbody>
          {players?.map(p=> (
            <tr key={p.id} className='fade-in border-t border-gray-800'>
              ...
              <td className='py-2'><RankPill rank={p.rank}/></td>
              <td className='py-2 text-gray-400'>{new Date(p.created_at).toLocaleString()}</td>
            </tr>
          ))}
